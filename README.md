...
通过浏览器可见页面操作，不构造或重放逆向签名请求，不收集账号密码，不接管日常浏览器资料。提交时可被动读取当前页面发出的响应，用于确认业务结果。不提供短信验证码、自动破解验证、无限采集、群发互动、私信、交易或付费功能；不等于抖音平台的全部功能。
...
启动日志不会打印 token，只提示文件位置。自行读取 `data/api-token` 并配置到可信的客户端；不要把真实值发到聊天、Issue 或截图里。也可通过 `DY_AUTH_TOKEN` 提供至少 32 个字符的随机令牌。启动时不会立即打开浏览器，首次登录或发布操作才会启动。
...
- 检测到平台验证时，保留当前专用页面，并尝试激活对应标签页、恢复最小化窗口；交由用户手动验证，不点短信发送、不填验证码。操作系统可能限制前台焦点；无头模式不能凭空显示窗口，需使用已有可见专用会话。
...
| `--headless` | `DY_HEADLESS` | `false`，显示专用浏览器 |
