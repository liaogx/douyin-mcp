...
首次搜索通过抖音主页的真实输入框和“搜索”按钮触发，后续关键词/筛选变更复用同一个已加载的标签页，不直接拼接带关键词的地址。每选择一项筛选，都等待结果稳定后再选下一项；菜单临时关闭时重新悬停展开，点击型筛选入口则只做一次有限点击恢复，读取前后复核全部选中条件。“相关搜索”推荐词卡片不计入作品。

如果首次按钮点击在页面 hydration 或代理瞬态下没有触发路由变化，服务会在确认没有进入当前关键词页后重新获取按钮并恢复点击一次；浏览器打开页面遇到有限的连接关闭/重置也只重试一次。两种恢复都不适用于登录、安全验证或评论、点赞等写操作。
